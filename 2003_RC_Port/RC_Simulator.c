         ...
         while(1)
         {
            ...
            if(pkt_size>4)  /* Size of version field in the struct */
            {
               ...
               switch(p_header->packet_type)
               {
                  ...
                  {
                     FrcIoData* p_io_data;
                     UINT8      masked_sw_dataA;
                     UINT8      masked_sw_dataB;
                     ...
                     /* First clear any bits in the packet that are unmasked */
                     masked_sw_dataA = p_io_data->rc_swA_mask & p_io_data->rc_swA;
                     masked_sw_dataB = p_io_data->rc_swB_mask & p_io_data->rc_swB;

                     /* Now clear all the bits from the RC data that are being changed */
                     s_sensor_data.rc_swA = s_sensor_data.rc_swA & (~(p_io_data->rc_swA_mask));
                     s_sensor_data.rc_swB = s_sensor_data.rc_swB & (~(p_io_data->rc_swB_mask));

                     /* Finally OR in the bits that are being set */
                     s_sensor_data.rc_swA = s_sensor_data.rc_swA | p_io_data->rc_swA_mask;
                     s_sensor_data.rc_swB = s_sensor_data.rc_swB | p_io_data->rc_swB_mask;
...
void Putdata(tx_data_ptr ptr)
{
   ...
   io_pkt.data.rc_swA_mask = 0;     /* No valid switch data in this packet */
   io_pkt.data.rc_swB_mask = 0;     /* No valid switch data in this packet */
