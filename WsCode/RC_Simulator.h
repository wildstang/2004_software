...
#ifndef __RC_SIMULATOR_H
#define __RC_SIMULATOR_H
...
/* Emulate serial port2 */
void          write_serial2(UINT8 data);
unsigned char recv_from_serial2(void);

/* Call this at the end of the robot main loop to let the simulator
 * do some things...like yield to other processes.
 */
void simulator_end_main_loop(void);

void Open2USART(UINT8 mode, UINT8 speed);
#define strcpypgm2ram(dst, src) strcpy(dst, src)
#define Delay1KTCYx(time)
